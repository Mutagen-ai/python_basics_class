...
# ================================================
# PHASE 3: BUILDING THE GUI WITH TKINTER
# ================================================

# frame to hold the 9 buttons in a grid
frame = tk.Frame(window)
frame.pack()

# create 9 buttons and place them in a 3x3 grid
buttons = []
for i in range(9):
    btn = tk.Button(
        frame,
        ...
        width=5,
        height=2,
        command=lambda i=i: click(i)   # each button knows its own index
    )
    btn.grid(row=i // 3, column=i % 3)
    buttons.append(btn)

# restart button below the board
tk.Button(window, text="Restart", font=("Arial", 12), command=reset).pack(pady=10)
